...
- **`backend/setup_pixeltable.py`** — Schema + data: tables, title embedding index, Analyze API attributes, scene detection (`scene_detect_histogram`), `video_scenes` view (`video_splitter` with `mode='fast'`). The `scene_marengo` embedding index is built directly on the `video_segment` column (Marengo 3.0) — a single embed pass at setup. Best-effort try/except on scene view.
...
Fully implemented. FastAPI + Pixeltable backend with scene-based video indexing. `scene_detect_histogram` finds natural scene boundaries, `video_splitter(mode='fast')` splits at those points (stream copy, no re-encoding), and the `scene_marengo` embedding index embeds each scene **once** directly on the `video_segment` column (Marengo 3.0). Search embeds the query (`similarity(string=...)`); recommendations read a watched video's stored scene vectors back out of the index via `video_segment.embedding(idx="scene_marengo")` and query with `similarity(vector=...)` for video-to-video matching with **no query-time upload** to Twelve Labs (~0.2s) — no separate precomputed-vector column needed. All queries route through `video_scenes` when available, with `title_marengo` as fallback. Shared `_scene_similarity` / `_title_similarity` in `videos.py` eliminate code duplication. `_select_videos` fetches computed attrs in a single query pass. `_load_creators_map()` cached with 5-min TTL. Quick-start: 3 videos (~4 min setup). Full: 25 videos from TL index with HLS playback, 10 creators. Dependencies: `scenedetect`, `opencv-python-headless`.
