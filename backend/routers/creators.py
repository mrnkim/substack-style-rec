...
from routers.videos import _attach_attrs, _build_video_response, _load_creators_map, _select_videos
...
def list_creators():
    ...
    return CreatorsListResponse(data=[
        CreatorResponse(
            id=cid, name=info["name"], avatar_url=info["avatar_url"],
            description=info["description"], video_count=info["video_count"],
        )
...
def get_creator(creator_id: str):
    ...
    rows = list(
        _select_videos(videos_t, videos_t.where(videos_t.creator_id == creator_id)).collect()
    )
    _attach_attrs(rows, videos_t)

    return CreatorDetailResponse(
        creator=CreatorResponse(
            id=creator_id, name=info["name"], avatar_url=info["avatar_url"],
            description=info["description"], video_count=info["video_count"],
        ),
