"""Download video files from Cloudflare R2 mirror (bypasses YouTube bot detection).

Usage (from backend/ or in Render Shell):
    uv run download_from_r2.py          # 3 quick-start videos
    uv run download_from_r2.py --full   # all 25 videos
"""
...
# Map YouTube ID → R2 filename (title-based, with special chars normalized)
R2_FILES: dict[str, str] = {
    ...
    "Ed9OqOQAfBs": "Lessons from $80M CEO on Building a Strong Personal Brand.mp4",
    "sO4te2QNsHY": "What Is Branding 4 Minute Crash Course.mp4",
    "y7rvDA3MARk": "We interviewed the Mozart of Gen Z.mp4",
    "d18ud-4epP8": "Why the Biggest YouTube Family Just Went to Netflix： Jordan Matter.webm",
    "BFU1OCkhBwo": "AI Expert Here Is What The World Looks Like In 2 Years! Tristan Harris.mp4",
    "fpETS6q1Hww": "Daniel Priestley AI Will Make Plumbers Earn More Than Lawyers! (2029 PREDICTION).mp4",
    "Xm_PHZXGe-w": "Theyre Lying About Healthy Foods Sugar! Shocking New Research Thats Harming You.mp4",
...
def download_and_convert(youtube_id: str, output_dir: Path) -> Path | None:
    """Download from R2, convert to mp4 if needed (ffmpeg remux, no re-encode)."""
    ...
    if not r2_filename:
        logger.warning("  No R2 mapping for %s", youtube_id)
        return None
...
def main():
    parser = argparse.ArgumentParser(description="Download videos from R2 mirror")
    ...
    if not args.full:
        ...
        logger.info("Quick-start: downloading %d videos from R2", len(youtube_ids))
    else:
        logger.info("Full mode: downloading %d videos from R2", len(youtube_ids))

    success, failed = 0, 0
    for i, yt_id in enumerate(youtube_ids, 1):
        logger.info("[%d/%d] %s", i, len(youtube_ids), yt_id)
        if download_and_convert(yt_id, DOWNLOAD_DIR):
            success += 1
        else:
            failed += 1
    ...
    logger.info("Video files: %d (%.1f GB total)", len(downloaded), total_gb)
